...
Inherits HTTPResponse
	#tag Event
		Function TamperMessageBody(ByRef Data As String) As Boolean
		  ...
		  Runtime.Context = Me
		  AddHandler Runtime.CompilerError, AddressOf Me.CompileError
		  AddHandler Runtime.RuntimeError, AddressOf Me.RuntimeError
	...
	#tag Method, Flags = &h21
		Private Sub CompileError(Sender As RBScript, line as Integer, errorNumber as Integer, errorMsg as String)
		  Dim err As New RuntimeException
		  err.Message = "Script compilation error on line " + Str(line) + ": " + ScriptErrorMessage(errorNumber)
	...
	#tag Method, Flags = &h1000
		Sub Constructor(Path As String, ScriptSourceFile As FolderItem)
		  // Calling the overridden superclass constructor.
		  Me.Constructor(Path)
		  Dim bs As BinaryStream = BinaryStream.Open(ScriptSourceFile)
		  Me.ScriptSource = bs.Read(bs.Length)
		  bs.Close
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		 Shared Function ScriptErrorMessage(code as integer) As string
		  ...
		  static errors as Dictionary
		  If errors = Nil Then errors = New Dictionary( _
		  1: "Syntax error", _
	...
	#tag Method, Flags = &h21
		Private Sub SetCookie(Key As String, Value As String)
		  Dim c As New HTTPCookie(Key, Value)
		  Super.SetCookie(c)
		End Sub
	#tag EndMethod
